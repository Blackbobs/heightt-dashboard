...
export default function FinanceView() {
  ...
  const stats = useMemo(() => {
    ...
    const totalRevenue = overview?.recentPayments?.reduce(
    ...
    // For now, we'll use a placeholder or calculate from data if available
    const totalWithdrawals = overview?.totalHeld || 0;
    ...
    return [
      ...
      },
      {
        ...
        change: `${overview?.recentPayments?.length || 0} payments`,
        trend: "up" as const,
        icon: DollarSign,
      ...
      },
      {
        ...
        trend: dueStats.pending > 0 ? "neutral" as const : "up" as const,
        icon: Coins,
        iconBg: "bg-purple-50",
  ...
  const columns = useMemo<ColumnDef<any, any>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ getValue }) => (
          <span
            className={`status-badge ${String(getValue()).toLowerCase()}`}
          >
            {getValue()}
          </span>
        ),
    ...
    <div>
      <div className="page-head">
        <div className="title">
          ...
          <p>
            Route: <code>/platform/finance</code> • Platform-wide financial
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="card">
          <div className="card-body">
            ...
            <div className="text-2xl font-bold text-slate-900">
              ₦{(overview?.totalBalance || 0).toLocaleString()}
            </div>
            <div className="text-xs text-slate-400">
        ...
        <div className="card">
          <div className="card-body">
            ...
            <div className="text-2xl font-bold text-amber-600">
              ₦{(overview?.totalHeld || 0).toLocaleString()}
            </div>
          ...
          <div className="card-body p-0">
            <div className="table-responsive">
              <table className="table">
                ...
                <tbody>
                  {overview.recentPayments.map((payment: any, index: number) => (
                    <tr key={payment.id || index}>
                      <td className="font-bold">
                        {payment.amountFormatted || `₦${(payment.amount || 0).toLocaleString()}`}
                      </td>
